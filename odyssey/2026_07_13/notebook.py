...
def _(mo):
    ...
    Гомеровские киклопы живут без законов и без кораблей, но земля кормит их
    сама — без сева и пахоты (IX.106–111). Это заметно перекликается с описанием
    «золотого века» у Овидия (*Метаморфозы* I.89–112, пер. С. В. Шервинского):
    ...
    У Гомера эта же черта — не столько идиллия, сколько дикость: киклопы не
    знают ни собраний, ни судов, ни кораблей; каждый правит один, «друг до
    друга им дела нет» (IX.112–115). Овидиев золотой век и гомеровская земля
    киклопов рисуют похожую картину доцивилизационного изобилия — но с разным
    знаком.
    """)
    return
...
def _(mo):
    ...
    mo.vstack(
        [
            mo.hstack(
                [SHOW_ICTUS, mo.md(
                    "Икты (ударные слоги) каждой стопы выделены <b style=\'color:green\'>зелёным</b>."
                )],
