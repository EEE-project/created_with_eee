...
def _(eee, mo):
    ...
    _img = eee.magnify_image(mo, _thumb_path, raw_base="https://codeberg.org/EEE-project/created_with_eee/raw/branch/main/odyssey/2026_07_13", width=280)
    _cap = mo.md(
            ...
            "карликового слона (<i>Palaeoloxodon falconeri</i>) могла восприниматься "
            "древними как единственная гигантская глазница. Такие ископаемые находили "
            "в пещерах Сицилии, Мальты и Крита — там же, где античные авторы селили "
            "циклопов.</div>"
        )
...
def _(mo):
    ...
    ### χρύσεον γένος — Гесиод (VIII–VII вв. до н.э.)

    Древнейшее из известных описаний — у Гесиода, «Труды и дни» (109–119). Он
    называет этих людей, современников Крона, **χρύσεον γένος** — «золотой
    род/поколение» (*γένος* — «род, поколение, племя», не время как таковое:
    у Гесиода это именно смена *родов* людей, а не «веков» в строгом смысле).
    Они не знают ни труда, ни старости; землю кормит сама (пер. В. В. Вересаева):
    ...
    ### aurea aetas — Овидий (I в. до н.э. — I в. н.э.)

    Спустя примерно семь веков Овидий в «Метаморфозах» (I.89–112) впервые
    называет ту же эпоху именно **aurea aetas** — «золотой век» в привычном
    нам смысле (*aetas* — «век, эпоха, время жизни»). Само словосочетание
    «золотой век» восходит скорее к латинской формулировке Овидия, чем к
    греческому *γένος* Гесиода, хотя сюжет — гесиодовский. Пер. С. В. Шервинского:
    ...
    Гомеровские киклопы живут той же спонтанной щедростью земли (IX.109:
    *τά γ᾽ ἄσπαρτα καὶ ἀνήροτα πάντα φύονται* — «всё растёт несеяное и
    непаханое само»), без сева и пахоты (IX.106–111) — но без богов рядом и
...
def _(mo):
    _EDITION = (
        "<b>Homeri Odyssea.</b> Ed. P. von der Mühll. "
...
def _(
    ...
    _vocab_path = Path(__file__).parent / "vocab_IX_105-129.tsv"
...
def _(cfg, gu):
    ...
    NB_REMOTE = f"{cfg.raw_base}/2026_07_13"
    for _f in (
        'Od_IX_105-129.pdf',
        'Od_IX_105-129_vocabula.pdf',
    ):
