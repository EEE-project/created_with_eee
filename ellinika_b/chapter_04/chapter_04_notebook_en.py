...
def _(mo):
    mo.md("""
    # «Είναι πανάκριβα!»
    ## Chapter 4 — Shopping & Prices

    **Textbook:** Ελληνικά Β' &nbsp;|&nbsp; **Level:** B1 &nbsp;|&nbsp; **Tests:** Nouns · Verbs · Adjectives

    ---

    ### Grammar: Adjectives ending in -ύς, -ιά, -ύ
    ...
    | Gen. Sg. | του φαρδιού | της φαρδιάς | του φαρδιού |
    ...
    Other adjectives of this type: **βαθύς** (deep), **μακρύς** (long)

    ---

    ### Grammar: Adjectives ending in -ής, -ιά, -ί (colors)

    These adjectives are mostly colors. Example: **ο καφετής** (brown)
    ...
    Colors: **πορτοκαλής** (orange), **βυσσινής** (maroon), **θαλασσής** (aqua), **χρυσαφής** (golden)

    ---

    ### Phonetics: Synizesis (Συνίζηση)

    Two adjacent vowels merge into one syllable: /i/ + /a, e, o, u/ → one syllable.

    Examples: **παιδιά** (children), **ποια** (which), **γεια** (hello), **εισιτήρια** (tickets)
...
def _(gu, mo, noun_form, noun_trans, noun_word, words4test_noun, words_noun):
    ...
    if words4test_noun() and noun_word:
        ...
        _view_simple = mo.vstack([
            mo.md(f"**Simple test for Nouns** ({len(words4test_noun())}/{len(words_noun)})"),
            mo.md(f"Translation: **{noun_trans}**"),
            noun_form,
            mo.md(_feedback),
        ])
    else:
        _view_simple = mo.md("**The word list is empty.** Select words in the table above.")
    _view_simple
    return
...
def _(
    ...
    if words4test_noun() and art_noun_word:
        ...
        _view_art = mo.vstack([
            mo.md(f"**Test for Nouns with Articles** ({len(words4test_noun())}/{len(words_noun)})"),
            mo.md(f"Translation: **{art_noun_trans}**"),
            art_noun_form,
            mo.md(_feedback_art),
        ])
    else:
        _view_art = mo.md("**The word list is empty.** Select words in the table above.")
    _view_art
    return
...
def _(cv_verb, gu, mo, tense_selector, verb_form, words4test_verb, words_verb):
    ...
    if not words4test_verb():
        _view_verb = mo.md("**The word list is empty.** Select words in the table above.")
    elif not _tense_key:
        _view_verb = mo.md("Select a tense above.")
    else:
        _feedback = ""
        if cv_verb:
        ...
        _view_verb = mo.vstack([
            mo.md(f"**Verb test — {_label}** ({len(words4test_verb())}/{len(words_verb)})"),
...
def _(gu, tense_selector, words4test_verb, words_verb):
    ...
    _tense_key = tense_selector.value
    _ui_label = f"{gu.TENSE_LABELS[_tense_key]['english']} ({gu.TENSE_LABELS[_tense_key]['greek']})" if _tense_key else "Select a tense"
...
def _(adj_cv, adj_form, adj_words, adj_words4test, gu, mo):
    ...
    if adj_words4test() and _adj:
        ...
        _view_adj = mo.vstack([
            mo.md(f"**Test: Adjective Declension** ({len(adj_words4test())}/{len(adj_words)})"),
            mo.md(f"Translation: **{_adj['Translation']}**"),
            adj_form,
            mo.md(_msg.replace("\n\n", "  \n")) if _msg else mo.md(""),
        ])
    else:
        _view_adj = mo.md("**The word list is empty.** Select words in the table above.")
    _view_adj
    return
