...
def _(language_selector, mo):
    ...
    _badge = "[![Open in molab](https://molab.marimo.io/molab-shield.svg)](https://molab.marimo.io/notebooks/nb_Q2MMHLzdNhY7LyUVHnDBZV)"
    if _lang == "ru":
        _out = mo.md(f"""
...
|:----------|:--------|:-----------|
| Φοβάμαι **ότι / πως** + Οριστική | Βεβαιότητα για αρνητική έκβαση | Φοβάμαι **ότι** θα χάσουμε το τρένο. |
| Φοβάμαι **μήπως / μη(ν)** + Υποτακτική | Ανησυχία ότι μπορεί να συμβεί κάτι | Φοβάμαι **μήπως** χάσουμε το τρένο. |
| Φοβάμαι **να** + Υποτακτική | Φόβος να κάνει κάτι ο ίδιος | Φοβάμαι **να** ανεβώ στη μηχανή του. |
""")
    else:
        _out = mo.md("""
...
| | |
|:--|:--|
| **Λέει να** πάει με τα πόδια. Δεν είναι πολύ μακριά. | *(Σκέφτεται να πάει...)* — πρόθεση, όχι βεβαιότητα |
| **Λέει ότι** θα πάει με τα πόδια, γιατί δεν είναι πολύ μακριά. | *(Λέει ότι θα πάει...)* — δηλώνει γεγονός |
...
| Μία φορά / ολοκληρωμένο — **Απλή Υποτακτική** | Διαρκές / επαναλαμβανόμενο — **Συνεχής Υποτακτική** |
|:------------------------------------------------|:-----------------------------------------------------|
| Μπορείς **να φας** ό,τι θέλεις σήμερα. | Μπορείς **να τρως** ό,τι θέλεις από αύριο. |
| Πρέπει **να πας** στο νοσοκομείο. | Πρέπει **να πηγαίνεις** τακτικά στον οδοντίατρο. |
| Θέλω **να περπατήσω** λίγο. | Θέλω **να περπατάω** μία ώρα κάθε μέρα. |
...
> **Απλή** = ίδιο θέμα με τον Απλό Μέλλοντα (θα φάω → να **φάω**)
> **Συνεχής** = ίδιο θέμα με τον Ενεστώτα (τρώω → να **τρώω/τρως**)
...
| Once / completed — **Απλή Υποτακτική** | Ongoing / repeated — **Συνεχής Υποτακτική** |
|:-----------------------------------------|:---------------------------------------------|
| Μπορείς **να φας** ό,τι θέλεις σήμερα. | Μπορείς **να τρως** ό,τι θέλεις από αύριο. |
| Πρέπει **να πας** στο νοσοκομείο. | Πρέπει **να πηγαίνεις** τακτικά στον οδοντίατρο. |
| Θέλω **να περπατήσω** λίγο. | Θέλω **να περπατάω** μία ώρα κάθε μέρα. |
...
> **Απλή** = same stem as Simple Future (θα φάω → να **φάω**)
> **Συνεχής** = same stem as Present (τρώω → να **τρώω/τρως**)
...
| **είναι πιθανό να** | Είναι **πιθανό να** πάθεις ατύχημα. |
| **είναι απίθανο να** | Είναι **απίθανο να** τρακάρεις. |
...
|:------|
| Ω, Θεέ μου! |
| Ρε άνθρωπέ μου, στραβός είσαι; |
| Ολόκληρο ΣΤΟΠ δεν το είδες; |
| Φοβάμαι ότι το έσπασα. |
| Ευτυχώς που φορούσε κράνος. |
| Εμ, βέβαια! Αν μιλάς στο κινητό, πώς να προσέξεις τη μηχανή; |
| Θα μπλέξετε άσχημα. |
| Μπορεί να το πληρώσετε ακριβά. |
| Πώς αισθάνεσαι; |
...
def _(gu, language_selector, mo, t_ui):
    # Tense selector — language-dependent, defaults to future (Απλή Υποτακτική)
    _lang = language_selector.value
    if _lang == "ru":
        _opts = {
            f"{gu.TENSE_LABELS['future']['greek']} (Απλή Υποτακτική / Простое сослагательное)": "future",
            f"{gu.TENSE_LABELS['present']['greek']} (Συνεχής Υποτακτική / Длительное сослагательное)": "present",
            f"{gu.TENSE_LABELS['aorist']['greek']} (Аорист)": "aorist",
            f"{gu.TENSE_LABELS['imperfect']['greek']} (Имперфект)": "imperfect",
        }
        _default = f"{gu.TENSE_LABELS['future']['greek']} (Απλή Υποτακτική / Простое сослагательное)"
    elif _lang == "el":
        _opts = {
            f"{gu.TENSE_LABELS['future']['greek']} (Απλή Υποτακτική)": "future",
            f"{gu.TENSE_LABELS['present']['greek']} (Συνεχής Υποτακτική / Ενεστώτας)": "present",
            f"{gu.TENSE_LABELS['aorist']['greek']} (Αόριστος)": "aorist",
            f"{gu.TENSE_LABELS['imperfect']['greek']} (Παρατατικός)": "imperfect",
        }
        _default = f"{gu.TENSE_LABELS['future']['greek']} (Απλή Υποτακτική)"
    else:
        _opts = {
            f"{gu.TENSE_LABELS['future']['greek']} (Simple Subjunctive / Simple Future)": "future",
            f"{gu.TENSE_LABELS['present']['greek']} (Continuous Subjunctive / Present)": "present",
            f"{gu.TENSE_LABELS['aorist']['greek']} (Simple Past)": "aorist",
            f"{gu.TENSE_LABELS['imperfect']['greek']} (Imperfect)": "imperfect",
        }
        _default = f"{gu.TENSE_LABELS['future']['greek']} (Simple Subjunctive / Simple Future)"
