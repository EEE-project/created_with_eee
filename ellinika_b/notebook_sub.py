...
def _(lang_sel, mo):
    ...
    _title, _subtitle = _TITLES.get(lang_sel.value, _TITLES["en"])
    mo.Html(f"""
    <style>
...
def _(lang_sel, mo, pd):
    ...
    _ROOT = "https://codeberg.org/EEE-project/created_with_eee/raw/branch/main"
    _lessons_df = pd.read_csv(f"{_ROOT}/ellinika_b/lessons.tsv", sep="\t", keep_default_na=False)
    _LESSONS = [
        {
            "icon": _row["icon"],
            "nb": _row["nb_id"],
    ...
    _cards = []
    for _l in _LESSONS:
        _url = f"https://molab.marimo.io/notebooks/{_l['nb']}/app" if _l["nb"] else None
        _label = _l['label'].get(_lang, _l['label']['en'])
