...
app = marimo.App(width="medium", app_title="Одиссея с Гомером — День 2: Одиссея IX.39–61")
...
def _(eee, mo):
    ...
    _right = eee.magnify_image(mo, _thumb_path, raw_base="https://raw.githubusercontent.com/EEE-project/created_with_eee/main/ancient_greek/odyssey/2026_06_22", width=280)
...
def _(
    ...
    _tp_path = gu.ensure_file(
        "translation_presence.tsv", nb_dir=_P(__file__).parent, remote_base=cfg.nb_remote("2026_06_22"),
    )
...
async def _(cfg, eee):
    ...
    _session_remote = cfg.nb_remote("2026_06_22")
...
def _(
    ...
    QUIZ_WORDS_RAW = gu.resolve_word_grammar(
        gu.load_inflected_vocab_tsv("vocab_IX_39-61.tsv", nb_dir=Path(__file__).parent, remote_base=cfg.nb_remote("2026_06_22")),
        ag_backend, "ru"
    )
...
def _(cfg, gu):
    ...
    NB_REMOTE = cfg.nb_remote("2026_06_22")
    for _f in (
        'Od_IX_39-61.pdf',
...
def _(cfg, mo):
    ...
    _prev_url, _next_url = cfg.adjacent_urls("2026_06_22/")
