...
app = marimo.App(width="medium", app_title="Одиссея с Гомером — День 3: Одиссея IX.62–81")
...
def _(
    ...
    _tp_path = gu.ensure_file(
        "translation_presence.tsv", nb_dir=_P(__file__).parent, remote_base=cfg.nb_remote("2026_06_29"),
    )
...
async def _(cfg, eee):
    ...
    _session_remote = cfg.nb_remote("2026_06_29")
...
def _(
    ...
    QUIZ_WORDS_RAW = gu.resolve_word_grammar(
        gu.load_inflected_vocab_tsv("vocab_IX_62-81.tsv", nb_dir=Path(__file__).parent, remote_base=cfg.nb_remote("2026_06_29")),
        ag_backend, "ru"
    )
...
def _(cfg, gu):
    ...
    NB_REMOTE = cfg.nb_remote("2026_06_29")
    for _f in (
        'odysseus_bust.jpg',
        'prepositions.jpg',
        'malea_cliff.jpg',
...
def _(cfg, mo):
    ...
    _prev_url, _next_url = cfg.adjacent_urls("2026_06_29/")
