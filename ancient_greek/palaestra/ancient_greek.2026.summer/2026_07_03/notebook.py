...
def _(mo):
    mo.md(r"""
    # Δίδαγμα η' · Κεφάλαιον II
    **Palaestra — Древнегреческий язык, начальный уровень — Лето 2026**

    [![Open in molab](https://molab.marimo.io/molab-shield.svg)](https://molab.marimo.io/notebooks/nb_L3ynoEAdmdKDC3qNbbT1vg)
    """)
    return
...
def _(NB_REMOTE, mo):
    mo.md(fr"""
    ## Проверка домашнего задания · Κεφ. II — τέλος

    Ответы к упражнениям начала II главы:

    [κεφΙΙ(0)\_ἀσκήματα(τέλος).pdf]({NB_REMOTE}/κεφΙΙ(0)_ἀσκήματα(τέλος).pdf)
    """)
    return
...
def _(mo):
    ...
    **μέν** и **δέ** не обязаны стоять в одном предложении: ответное **δέ** может
    прийти и в следующей фразе.
...
def _(mo):
    mo.md(r"""
    ## Связки, причина, слитные глаголы

    **Ударение у слитных глаголов** — не на привычном месте:
    обычные *ἐκβαίνει, καθεύδει*, но слитные *καλεῖ, πονεῖ*.

    **Связки** (перечисление): *Α, Β, Γ* или *Α καὶ Β καὶ Γ*.
    Как по-русски («папа, мама и я») — нельзя.

    **Причинно-следственная связь:** *γάρ … οὖν*.
    """)
    return
...
def _(mo):
    ...
    **οὕτω** — так (наречие): *μὴ οὕτω χαλεπός! · μὴ οὕτω ἀργός!*

    **οὐ μόνον …, ἀλλὰ καὶ …** — не только …, но и …

    **ἔτι** — ещё (сплю) · **οὐκέτι** — уже не (*no longer*) · **οὔπω** — ещё не

    Энклитики — ударение на втором слоге.
    """)
    return
...
def _(NB_DIR, NB_REMOTE, gu):
    ...
    for _pdf in (
        'κεφΙΙ(1).pdf',
        'Κεφ.II(2)_μετάφρασις.pdf',
        'Κεφ.II(1)_μετάφρασις_τέλος.pdf',
        'κεφΙΙ(0)_ἀσκήματα(τέλος).pdf',
...
def _(cfg, mo):
    ...
    NB_REMOTE = cfg.nb_remote("2026_07_03")
