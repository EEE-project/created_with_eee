...
help:
	...
	@echo "                      index.tsv-driven) are not regenerated by this target. Then"
	@echo "                      run 'make sync-pages-export' with the same NOTEBOOKS to"
	@echo "                      copy the export to every other host's checkout."
	@echo "  sync-pages-export   Copy NOTEBOOKS' export from PAGES_DIR into GITHUB_PAGES_DIR /"
	@echo "                      GITLAB_UNIFIED_PAGES_DIR / SPLIT_PROJECTS_DIR checkouts (see"
	@echo "                      tools/sync-pages-export.py for the per-host layout rules --"
	@echo "                      split-off courses land in their own GitLab project instead"
	@echo "                      of GitLab-unified). Run 'make export-notebooks' first. Only"
...
export-notebooks:
	...
	@echo "sync-pages-export' with the same NOTEBOOKS to copy it everywhere else (the"
	@echo "export itself is host-portable, no need to re-run per host)."

sync-pages-export:
	$(EEE_PYTHON) tools/sync-pages-export.py --source-dir $(PAGES_DIR) \
