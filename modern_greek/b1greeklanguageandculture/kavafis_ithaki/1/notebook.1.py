...
def _(
    ...
    LITERARY_TRANSLATORS = ["Шмаков/Бродский", "Ильинская", "Левитов", "Keeley/Sherrard"]
    _tp_vocab = [w for w in POEM_WORDS_RAW if w.get("pos") in eee.TRANSLATION_PRESENCE_CONTENT_POS]
    _tp_path = os.path.join(notebook_dir, "translation_presence.tsv")
...
@app.cell(hide_code=True)
def _(notebook_dir, os, pd):
    # Vocabulary data (useful expressions + literary terms)
    df_vocab = pd.read_csv(os.path.join(notebook_dir, 'vocabulary.tsv'), sep='\t')
    return (df_vocab,)
...
def _(language_selector, mo, t_ui):
    trans_selector = mo.ui.dropdown(
        options={
            ...
            "Левитов · рус.": "Левитов",
            "Keeley / Sherrard (1975) · англ.": "Keeley/Sherrard",
        },
        value="подстрочник",
...
def _(eee):
    ...
    _greek = eee.parse_stanza_text((_root / "greek.md").read_text(encoding="utf-8"))
    _trans, _desc = eee.parse_stanza_translations((_root / "translations.md").read_text(encoding="utf-8"))
    TRANS_DESC = _desc
    STANZAS = [
        {
            "ref": ref,
...
def _(notebook_dir, os):
    import csv as _csv
    with open(os.path.join(notebook_dir, "poem_vocab.tsv"), encoding="utf-8") as _f:
        POEM_WORDS_RAW = list(_csv.DictReader(_f, delimiter="\t"))
...
def _():
    ...
    return eee, gu2, mo, notebook_dir, os, pd, random
