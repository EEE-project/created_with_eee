...
def _(df_noun, mo, tbl_sel_n):
    ...
    mo.vstack([
        mo.md("### Select nouns to practise"),
        table_noun,
    ])
    return (table_noun,)
...
def _(
    ...
    if words4test_noun() and noun_word:
        ...
        _view = mo.vstack([
            mo.md(f"**Nouns** ({len(words4test_noun())}/{session_total_n()})"),
        ...
        _view = mo.md("_Select nouns from the table above._")
    _view
    return
...
def _(
    ...
    if words4test_noun() and art_noun_word:
        ...
        _view_art = mo.vstack([
            mo.md(f"**Nouns with article** ({len(words4test_noun())}/{session_total_n()})"),
        ...
        _view_art = mo.md("_Select nouns from the table above._")
    _view_art
    return
...
def _(df_verb, mo, tbl_sel_v):
    ...
    mo.vstack([mo.md("### Select verbs to practise"), table_verb])
    return (table_verb,)
...
def _(gu, mo):
    ...
    _tense_options = {label: key for label, key in gu.tense_dropdown_options('en').items() if key in _b1_tenses}
    tense_selector = mo.ui.dropdown(
        options=_tense_options,
        value=next(label for label, key in gu.tense_dropdown_options('en').items() if key == 'present'),
        label="Select tense:",
    )
    tense_selector
...
def _(
    ...
    if not words4test_verb():
        _view_verb = mo.md("_Select verbs from the table above._")
        ...
        _label = gu.TENSE_LABELS.get(tense_selector.value, {}).get('label', {}).get('en', tense_selector.value)
        _items = [mo.md(f"**Verbs — {_label}** ({len(words4test_verb())}/{session_total_v()})")]
        if verb_msg():
            _items.append(mo.md(verb_msg()))
        _items += [
            mo.md(f"Translation: **{_cv['Translation']}**") if _cv else mo.md(""),
            verb_fields,
...
def _(
    ...
    if _cv and _tense_key and _c and getattr(_c, 'verb_word', None) == _cv['Word'] and getattr(_c, 'tense', None) == _tense_key:
        ...
        if _ok:
            ...
            set_verb_msg(f'<span style="color:green;">Test for <b>"{_cv["Word"]} — {_cv["Translation"]}"</b> passed.\n\n{len(_new)} words remaining out of {session_total_v()}.</span>')
...
def _(df_adj, mo, tbl_sel_a):
    ...
    mo.vstack([mo.md("### Select adjectives to practise"), table_adj])
    return (table_adj,)
...
def _(
    ...
    if words4test_adj() and _adj:
        ...
        _view_adj = mo.vstack([
            mo.md(f"**Adjectives** ({len(words4test_adj())}/{session_total_a()})"),
        ...
        _view_adj = mo.md("_Select adjectives from the table above._")
    _view_adj
    return
