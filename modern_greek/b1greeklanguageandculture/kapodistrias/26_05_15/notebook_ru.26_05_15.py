...
def _(
    ...
    _clk = lambda v: (v or 0) + 1
    submit_button_n = mo.ui.button(label="Отправить", on_click=_clk, kind="warn" if _dirty else "neutral")
...
def _(
    ...
    if words4test_noun() and noun_word:
        ...
        _view = mo.vstack([
            mo.md(f"**Простой тест: Существительные** ({len(words4test_noun())}/{session_total_n()})"),
...
def _(
    ...
    if words4test_noun() and art_noun_word:
        ...
        _view_art = mo.vstack([
            mo.md(f"**Тест с артиклями: Существительные** ({len(words4test_noun())}/{session_total_n()})"),
...
def _(gu, mo):
    _RU_LABELS = {
        "present": "Настоящее", "imperfect": "Имперфект",
        "aorist": "Аорист", "future": "Простое будущее",
        "future_continuous": "Длительное будущее",
    }
...
def _(captured_verb, mo, set_submit_count_v, tense_selector, verb_fields):
    ...
    _clk = lambda v: (v or 0) + 1
    submit_button_v = mo.ui.button(label="Отправить", on_click=_clk, kind="warn" if _dirty else "neutral")
...
def _(
    ...
    if not words4test_verb():
        ...
        _label = _RU_LABELS.get(tense_selector.value, tense_selector.value)
        _items = [mo.md(f"**Тест глагола — {_label}** ({len(words4test_verb())}/{session_total_v()})")]
        if verb_msg():
...
def _(adj_form, captured_adj, mo, set_submit_count_a):
    ...
    _dirty = _has_input and not _matches_snap
    _clk = lambda v: (v or 0) + 1
    submit_button_a = mo.ui.button(label="Отправить", on_click=_clk, kind="warn" if _dirty else "neutral")
