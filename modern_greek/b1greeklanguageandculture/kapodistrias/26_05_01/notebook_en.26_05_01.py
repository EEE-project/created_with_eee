...
def _(df_noun, gu, mo, tbl_sel_n):
    ...
    mo.vstack([
        mo.md("### Select nouns to practice"),
        table_noun,
    ])
    return (table_noun,)
...
def _(df_verb, gu, mo, tbl_sel_v):
    ...
    mo.vstack([mo.md("### Select verbs to practice"), table_verb])
    return (table_verb,)
...
def _(clear_count_v, cv_verb, gu, tense_selector, words4test_verb, words_verb):
    ...
    _ui_label = gu.TENSE_LABELS.get(_tense_key, {}).get('label', {}).get('en', _tense_key) if _tense_key else "—"
...
def _(
    ...
    if not words4test_verb():
        ...
        _items = [mo.md(f"**Verb test — {_label}** ({len(words4test_verb())}/{session_total_v()})")]
        if verb_msg():
...
def _(df_adj, gu, mo, tbl_sel_a):
    ...
    mo.vstack([mo.md("### Select adjectives to practice"), table_adj])
    return (table_adj,)
...
def _(
    ...
    if words4test_adj() and _adj:
        ...
        _view_adj = mo.vstack([
            mo.md(f"**Test: Adjective declension** ({len(words4test_adj())}/{session_total_a()})"),
