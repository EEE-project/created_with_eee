...
def _(mo, notebook_dir, os):
    ...
    mo.vstack([
        mo.md(r"""
        # **Иоаннис Каподистриас: Первый Правитель Греции**
        Ниже представлено краткое изложение информации об Иоаннисе Каподистриасе из [презентации](https://docs.google.com/presentation/d/1Za9Z9vyZDUYFn_Rv_QZBga_GxfO5fn9I/edit?usp=sharing&ouid=114390708685640574713&rtpof=true&sd=true) и фильма «Иоаннис Каподистриас — Невероятная история первого правителя Греции» ([субтитры](https://drive.google.com/file/d/1z-3NR6gyGQg4kugiOVovggtP4_UWoIKh/view?usp=drive_link)):
        """),
        _img(1),
        mo.Html('''<iframe
            width="560"
        ...
        *   Его фамилия имеет итальянское происхождение (**Capo d'Istria**), что означает «из Истрии» — региона в современной Словении/Хорватии, а его первоначальная фамилия была «Витторе».
        """),
        _img(3),
        _img(4),
        mo.md(r"""
        ## **2. Дипломатическая карьера в России**
        *   В 1808 году он эмигрировал в **Россию**, где работал дипломатом.
        *   Стал одним из ведущих дипломатов Европы, дослужившись до должности **министра иностранных дел царя Александра I**.
        *   Несмотря на богатую и безопасную жизнь в России, он решил бросить всё и помочь Греции после революции.
        """),
        _img(5),
        mo.md(r"""
        ## **3. Управление Грецией (1828–1831)**
        *   Прибыл в Грецию в **1828 году** и обнаружил страну в состоянии полного хаоса — без армии, казны, дорог и школ.
        *   **Главные достижения:**
        ...
        *   **Личная позиция:** Отказался от жалованья и использовал личное состояние для поддержки сирот и вдов.
        """),
        _img(6),
        mo.md(r"""
        ## **4. Конфликты и оппозиция**
        *   Политика **сильного централизованного государства** вызвала гнев местных вождей и влиятельных семей, таких как клан **Мавромихали**, потерявших свои привилегии.
        *   Иностранные державы относились к нему с подозрением, желая иметь слабую и управляемую Грецию.

        ## **5. Убийство и наследие**
        *   Был убит утром **27 сентября 1831 года** в Нафплионе, у церкви Святого Спиридона.
        *   Исполнители — Константинос и Георгиос Мавромихали.
        *   После его смерти страна снова погрузилась в хаос и гражданские конфликты. Гражданские войны, иностранные интервенции и в конечном счёте — король из-за рубежа.
        *   Главным наследием стала идея о том, что Греция может функционировать как **современное, организованное и честное государство**.
        """),
    ])
    return
...
@app.cell(hide_code=True)
def _(gu, mo, notebook_dir, os, pd):
    df_vocabulary = pd.read_csv(os.path.join(notebook_dir, 'vocabulary.tsv'), sep='\t')
    table_vocabulary = gu.vocab_table(df_vocabulary)
    mo.vstack([
        mo.md("## ПОЛЕЗНЫЕ СЛОВА"),
        table_vocabulary,
    ])
    return
...
def _(file_upload_noun, gu, notebook_dir):
    df_noun = gu.load_vocab_table("nouns.tsv", nb_dir=notebook_dir, ru_variant=True, language='ru', file_upload=file_upload_noun)
    return (df_noun,)
...
def _(file_upload_verb, gu, notebook_dir):
    df_verb = gu.load_vocab_table("verbs.tsv", nb_dir=notebook_dir, ru_variant=True, language='ru', file_upload=file_upload_verb)
    return (df_verb,)
...
def _(file_upload_adj, gu, notebook_dir):
    df_adj = gu.load_vocab_table("adjectives.tsv", nb_dir=notebook_dir, ru_variant=True, language='ru', file_upload=file_upload_adj)
    return (df_adj,)
...
def _():
    ...
    return eee, gu, mo, notebook_dir, os, pd, random
