...
def _(mo):
    ...
    **Grammar:** Continuous Future · Temporal Clauses
...
@app.cell(hide_code=True)
def _(mo):
    # Grammar: Για δες — tense comparison
    mo.md("""
    ## Grammar: Tenses in Context (Για δες)

    | | Past | | Past |
    |:--|:--|:--|:--|
    | one time | Χτες **διάβασα** εφημερίδα. _(Aorist)_ | ongoing / habitual | Παλιά **διάβαζα** εφημερίδα κάθε μέρα. _(Imperfect)_ |
    | | **Future** | | **Future** |
    | one time | Αύριο **θα διαβάσω** για το τεστ. _(Simple Future)_ | ongoing / habitual | Από αύριο **θα διαβάζω** τρεις ώρες την ήμερα. _(Continuous Future)_ |
...
def _(gu, mo):
    ...
    tense_selector = mo.ui.dropdown(
        ...
        value=f"{gu.TENSE_LABELS['future_continuous']['greek']} (Continuous Future)",
        label="Select tense:",
    )
    tense_selector
