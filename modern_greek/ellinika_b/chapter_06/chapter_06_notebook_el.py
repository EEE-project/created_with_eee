...
def _(mo):
    mo.md("""
    # «Φάγαμε, ήπιαμε…»
    ## Κεφάλαιο 6 — Φαγητό & Ποτά · B1

    **Βιβλίο:** Ελληνικά Β'
    **Γραμματική:** Ουσιαστικά (αρσ. -άς/-ές/-ης/-ής/-ούς · θηλ. -ά/-ού · ουδ. -ας) · Αόριστος
...
def _(mo):
    mo.md("""
    ## Γραμματική: Ουσιαστικά — Πληθυντικός σε -δες

    Μεγάλη ομάδα ουσιαστικών όλων των γενών σχηματίζει τον πληθυντικό με **-δες** (και όχι με τα συνηθισμένα -οι/-ες/-α).

    ### ΑΡΣΕΝΙΚΑ — 5 τύποι

    | Τύπος | Παράδειγμα | Εν. Γεν./Αιτ. | Πληθ. Ονομ. |
    |-------|------------|--------------|-------------|
    ...
    - **-άς:** ο ψαράς, ο ανανάς, ο κιμάς
    - **-ές:** ο μεζές, ο κεφτές
    - **-ης:** ο χασάπης, ο μπακάλης
    - **-ής:** ο μπουφετζής, ο καφετζής
    - **-ούς:** ο παππούς
    ...
    ### ΟΥΔΕΤΕΡΑ — ειδικοί τύποι

    **το κρέας** — κανονικό πρότυπο -ας/-ατα:

    | Πτώση | Ενικός | Πληθυντικός |
    |-------|--------|-------------|
    | Ονομ. | το κρέ**ας** | τα κρέ**ατα** |
    | Γεν. | του κρέ**ατος** | των κρε**άτων** |
    | Αιτ. | το κρέ**ας** | τα κρέ**ατα** |

    **το γάλα** — εξαίρεση: η γενική ενικού χρησιμοποιεί την αρχαία ρίζα **γαλακτ-**:

    | Πτώση | Ενικός | Πληθυντικός |
    |-------|--------|-------------|
    | Ονομ. | το γάλ**α** | τα γάλ**ατα** *(σπάνιο — μη μετρήσιμο)* |
    | Γεν. | του **γάλακτος** ← αρχαία ρίζα | *(δεν χρησιμοποιείται)* |
    | Αιτ. | το γάλ**α** | τα γάλ**ατα** *(= ονομ.)* |

    Πλήρης κλίση του **ο παππούς** (αντιπροσωπευτικός τύπος -ούς):

    | Πτώση | Ενικός | Πληθυντικός |
    |-------|--------|-------------|
    | Ονομ. | ο παππ**ούς** | οι παππ**ούδες** |
    | Γεν. | του παππ**ού** | των παππ**ούδων** |
    | Αιτ. | τον παππ**ού** | τους παππ**ούδες** |
...
def _(mo):
    mo.md("""
    ## Χρήσιμες εκφράσεις: Στο εστιατόριο
...
def _(df_noun, gu, mo, tbl_sel_n):
    ...
    mo.vstack([mo.md("### Επιλογή ουσιαστικών για εξάσκηση"), table_noun if table_noun is not None else mo.md("_Το αρχείο nouns.tsv δεν βρέθηκε — φόρτωσε αρχείο._")])
    return (table_noun,)
...
def _(
    ...
    if words4test_noun() and noun_word:
        ...
        _view = mo.md("_Επιλέξτε ουσιαστικά από τον παραπάνω πίνακα για να ξεκινήσετε._")
    _view
    return
...
def _(
    ...
    if words4test_noun() and art_noun_word:
        ...
        _view_art = mo.md("_Επιλέξτε ουσιαστικά από τον παραπάνω πίνακα για να ξεκινήσετε._")
    _view_art
    return
...
def _(df_verb, gu, mo, tbl_sel_v):
    ...
    mo.vstack([mo.md("### Επιλογή ρημάτων για εξάσκηση"), table_verb if table_verb is not None else mo.md("_Το αρχείο verbs.tsv δεν βρέθηκε — φόρτωσε αρχείο._")])
    return (table_verb,)
...
def _(clear_count_v, gu, random, tense_selector, words4test_verb, words_verb):
    ...
    _ui_label = gu.TENSE_LABELS[_tense_key]['greek'] if _tense_key else "Επιλέξτε χρόνο"
...
def _(
    ...
    if not words4test_verb():
        _view_verb = mo.md("_Επιλέξτε ρήματα από τον παραπάνω πίνακα για να ξεκινήσετε._")
...
def _(df_adj, gu, mo, tbl_sel_a):
    ...
    mo.vstack([mo.md("### Επιλογή επιθέτων για εξάσκηση"), table_adj if table_adj is not None else mo.md("_Το αρχείο adjectives.tsv δεν βρέθηκε — φόρτωσε αρχείο._")])
    return (table_adj,)
...
def _(mo):
    mode_selector = mo.ui.radio(
        options={
            ...
            "Σύνθετο: όλα τα γένη, αριθμοί και πτώσεις (18 πεδία)": "complex",
        },
        value="Απλό: 3 γένη × 2 αριθμοί (6 πεδία)",
        label="Επιλογή τρόπου:",
    )
...
def _(
    ...
    if words4test_adj() and _adj:
        ...
        _view_adj = mo.md("_Επιλέξτε επίθετα από τον παραπάνω πίνακα για να ξεκινήσετε._")
    _view_adj
    return
