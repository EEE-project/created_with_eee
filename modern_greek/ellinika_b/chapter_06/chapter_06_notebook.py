...
app = marimo.App(width="medium", app_title="Ελληνικά Β1 — Chapter 6: We Ate, We Drank…")
...
def _(language_selector, mo):
    ...
    Существительные мужского и женского рода ниже добавляют **-δ-** перед окончанием множественного числа (8 разных типов, сгруппированных по окончанию, — это не одно правило). Существительные среднего рода следуют отдельной, не связанной с этим моделью **-α → -ατα** без всякого -δ-. Каждый тип приведён точно по таблице учебника (Им./Род./Вин. × Ед./Мн.).
    ...
    **το γάλα** (неправильная модель — родительный падеж с необязательным -κ-, родительного падежа множественного числа в книге нет вообще — прочерк)
    ...
    """)
        ...
        _out = mo.md("""
    ## Γραμματική: Ουσιαστικά — Πληθυντικός σε -δες (σ. 88)

    Τα αρσενικά και τα θηλυκά ουσιαστικά παρακάτω προσθέτουν **-δ-** πριν από την κατάληξη του πληθυντικού (8 διαφορετικοί τύποι, ομαδοποιημένοι ανά κατάληξη — όχι ένας ενιαίος κανόνας). Τα ουδέτερα ουσιαστικά ακολουθούν ένα ξεχωριστό, άσχετο μοντέλο **-α → -ατα** χωρίς κανένα -δ-. Κάθε τύπος αναπαράγεται ακριβώς όπως στον πίνακα του βιβλίου (Ονομ./Γεν./Αιτ. × Ενικός/Πληθυντικός).
    ...
    **-άς → -άδες**
    ...
    |-------|--------|-------------|
    | Ονομ. | ο μπαμπά**ς** | οι μπαμπά**δες** |
    | Γεν. | του μπαμπά | των μπαμπά**δων** |
    | Αιτ. | τον μπαμπά | τους μπαμπά**δες** |

    Ίδιος τύπος: ψαράς (fisherman), παπάς (priest), χαλβάς (halva), μπακλαβάς (baklava), καβγάς (quarrel), βοριάς (north wind), νοτιάς (south wind), υπναράς (sleepyhead), περιπτεράς (kiosk owner). — _Δεν μπορώ άλλο τους **καβγάδες** σας! Γιατί δε συμφωνείτε ποτέ σε κάτι;_ ("I can't take any more of your quarrels! Why do you never agree on anything?")

    **-ές → -έδες**

    | Πτώση | Ενικός | Πληθυντικός |
    |-------|--------|-------------|
    | Ονομ. | ο καφ**ές** | οι καφ**έδες** |
    | Γεν. | του καφέ | των καφ**έδων** |
    | Αιτ. | τον καφέ | τους καφ**έδες** |

    Ίδιος τύπος: κεφτές (meatball), μεζές (appetizer), πουρές (mash), κανατές (jug-ful). — _Καλά, πόσους **καφέδες** ήπιες σήμερα;_ ("Seriously, how many coffees did you drink today?")

    **-ης → -ηδες**

    | Πτώση | Ενικός | Πληθυντικός |
    |-------|--------|-------------|
    | Ονομ. | ο μανάβ**ης** | οι μανάβ**ηδες** |
    | Γεν. | του μανάβη | των μανάβ**ηδων** |
    | Αιτ. | τον μανάβη | τους μανάβ**ηδες** |

    Ίδιος τύπος: μπακάλης (grocer), χασάπης (butcher), τσαγκάρης (shoemaker), σπιτονοικοκύρης (landlord). — _Ξέρεις τι μ' αρέσει στη λαϊκή; Όλη αυτή η φασαρία, οι φωνές των **μανάβηδων**…_ ("Know what I like about the street market? All the bustle, the greengrocers' shouting...")

    **-ής → -ήδες**

    | Πτώση | Ενικός | Πληθυντικός |
    |-------|--------|-------------|
    | Ονομ. | ο καφετζ**ής** | οι καφετζ**ήδες** |
    | Γεν. | του καφετζή | των καφετζ**ήδων** |
    | Αιτ. | τον καφετζή | τους καφετζ**ήδες** |

    Ίδιος τύπος: ψιλικατζής (kiosk owner), παλιατζής (junk dealer), ταξιτζής (taxi driver), φορτηγατζής (truck driver), μπογιατζής (house painter), σουβλατζής (souvlaki seller). — _Ξέρεις κανέναν καλό **μπογιατζή**; Θέλουμε να βάψουμε το σπίτι μετά το καλοκαίρι._ ("Do you know a good painter? We want to paint the house after summer.")
    ...
    **-ούς → -ούδες**
    ...
    | Γεν. | του παππού | των παππ**ούδων** |
    | Αιτ. | τον παππού | τους παππ**ούδες** |

    (μόνο το παππούς ακολουθεί αυτόν τον τύπο) — _Πήρες τηλέφωνο τον **παππού** να δεις τι κάνει; Ήταν λίγο άρρωστος χτες._ ("Did you call grandpa to see how he's doing? He was a bit sick yesterday.")
    ...
    **-ά → -άδες**
    ...
    | Πτώση | Ενικός | Πληθυντικός |
    |-------|--------|-------------|
    | Ονομ. | η μαμ**ά** | οι μαμ**άδες** |
    | Γεν. | της μαμάς | των μαμ**άδων** |
    | Αιτ. | τη μαμά | τις μαμ**άδες** |

    Ίδιος τύπος: γιαγιά (grandma). — _– Πού είναι το μωρό σας; Δεν το φέρατε μαζί; – Όχι, έμεινε στο σπίτι με τις **γιαγιάδες** του._ ("Where's your baby? Didn't you bring them along? – No, they stayed home with their grandmothers.")

    **-ού → -ούδες**

    | Πτώση | Ενικός | Πληθυντικός |
    |-------|--------|-------------|
    | Ονομ. | η μαϊμ**ού** | οι μαϊμ**ούδες** |
    | Γεν. | της μαϊμούς | των μαϊμ**ούδων** |
    | Αιτ. | τη μαϊμού | τις μαϊμ**ούδες** |

    Ίδιος τύπος: αλεπού (fox). — _Επιτέλους! Μετά από τόση ώρα στο βουνό είδαμε δύο **αλεπούδες**._ ("Finally! After so long in the mountains we saw two foxes.")

    ### ΟΥΔΕΤΕΡΑ — κανονικό vs. ανώμαλο -ας/-ατα

    **το κρέας** (κανονικό μοντέλο)
    ...
    **το γάλα** (ανώμαλο — η γενική διατηρεί προαιρετικό -κ-, και το βιβλίο δεν δίνει καθόλου γενική πληθυντικού)

    | Πτώση | Ενικός | Πληθυντικός |
    |-------|--------|-------------|
    | Ονομ. | το γάλ**α** | τα γάλ**ατα** |
    | Γεν. | του γάλα(**κ**)τος | – |
    | Αιτ. | το γάλ**α** | τα γάλ**ατα** |
    ...
    Masculine and feminine nouns below add a **-δ-** before the plural ending (8 distinct patterns, grouped by ending — not a single rule). Neuter nouns follow a separate, unrelated **-α → -ατα** pattern with no -δ- at all. Each type is reproduced exactly as the book's own table (Nom./Gen./Acc. × Singular/Plural).
...
def _(language_selector, mo):
    ...
    **-α, -ες, -ε, -αμε, -ατε, -αν** (ударение не может отступать дальше 3-го слога от конца).
    ...
    **→ -ψα** (настоящее оканчивается на **-εύω, -φω, -βω, -πω, -πτω**)
    ...
    | κάθομαι | **κάθισα** / έκατσα | | τρώω | **έφαγα** |
    ...
    **-α, -ες, -ε, -αμε, -ατε, -αν** (ο τόνος δεν μπορεί να πέσει πιο πριν από την 3η συλλαβή από το τέλος).
    ...
    **-α, -ες, -ε, -αμε, -ατε, -αν** (stress can occur no farther left than the 3rd syllable from the end).
