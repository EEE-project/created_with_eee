...
def _(mo):
    ...
    ## Ενότητα 7 — Εικόνες από το παρελθόν · B1

    **Γραμματική:** Παρατατικός (Α, Β1, Β2, Α/Β) · Χρονικοί σύνδεσμοι
...
def _(mo):
    # Grammar: Παρατατικός
    mo.md("""
    ## Γραμματική: Παρατατικός (Παρελθόν διαρκείας)

    Εκφράζει **διαρκείς ή επαναλαμβανόμενες** πράξεις στο παρελθόν.
    Χρονικές εκφράσεις: *κάθε μέρα*, *συχνά*, *πάντα*, *όταν ήμουν παιδί*.

    ### Τύπος Α — καταλήξεις **-α, -ες, -ε, -αμε, -ατε, -αν(ε)**

    | Πρόσωπο | παίζω | πηγαίνω |
    |:--------|:------|:--------|
    ...
    > Το κόκκινο <span style="color:red">**έ-**</span> είναι αύξηση που προστίθεται στα κοντά θέματα: γράφω→<span style="color:red">**έ**</span>γραφα, διαβάζω→**διά**βαζα.

    ### Τύπος Β1 (-άω) · δύο παραλλαγές · Τύπος Β2 (-ώ) · μία παραλλαγή

    | Πρόσωπο | περνάω (Β1) | μπορώ (Β2) |
    |:--------|:------------|:-----------|
    ...
    ### Τύπος Α/Β — εισαγωγή **-γ-** *(ακούω, καίω, λέω, κλαίω, τρώω, φταίω)*
    ...
    > Το κόκκινο <span style="color:red">**-γ-**</span> μπαίνει πριν από την κατάληξη: λέω→<span style="color:red">**έ</span>λε**<span style="color:red">**γ**</span>α, τρώω→<span style="color:red">**έ**</span>τρω<span style="color:red">**γ**</span>α
...
def _(mo):
    ...
    mo.md("""
    ## Γραμματική: Χρονικοί σύνδεσμοι με Παρατατικό

    ### Παρατατικός + Παρατατικός — δύο ταυτόχρονες διαρκείς πράξεις

    Σύνδεσμοι: **Όταν** (when), **Ενώ** (while), **Όση ώρα** (as long as).
    ...
    ### Παρατατικός + Αόριστος — διαρκής πράξη που διακόπτεται από σύντομο γεγονός

    Σύνδεσμοι: **Ενώ**, **Την ώρα που**, **Καθώς** (while / as).
...
def _(mo):
    ...
    Σύντομες λέξεις χάνουν το τελικό φωνήεν πριν από φωνήεν:
    ...
    | | → | Παράδειγμα |
    |:-|:--|:-----------|
    | μου + [ε-/ι-] | μου '- | μου έλεγε → μου 'λεγε |
    | σου + [ε-/ι-] | σου '- | σου έστελνε → σου 'στέλνε |
    | του + [ε-/ι-] | του '- | του έφερνε → του 'φερνε |
    | το + [ε-/ι-] | το '- | το ήξερε → το 'ξερε |
    | τα + [ε-/ι-] | τα '- | τα έδινε → τα 'δινε |
    """)
    return
...
def _(mo):
    # Noun file upload
    file_upload_noun = mo.ui.file(label="Φόρτωσε TSV ουσιαστικών")
    file_upload_noun
...
def _(df_noun, mo, tbl_sel_n):
    ...
    mo.vstack([mo.md("### Επίλεξε ουσιαστικά για εξάσκηση"), table_noun if table_noun is not None else mo.md("_Το αρχείο nouns.tsv δεν βρέθηκε — φόρτωσε αρχείο για να ξεκινήσεις._")])
    return (table_noun,)
...
def _(mo):
    # Verb file upload
    file_upload_verb = mo.ui.file(label="Φόρτωσε TSV ρημάτων")
    file_upload_verb
...
def _(df_verb, mo, tbl_sel_v):
    ...
    mo.vstack([mo.md("### Επίλεξε ρήματα για εξάσκηση"), table_verb if table_verb is not None else mo.md("_Το αρχείο verbs.tsv δεν βρέθηκε — φόρτωσε αρχείο για να ξεκινήσεις._")])
    return (table_verb,)
...
def _(gu, mo):
    ...
    tense_selector = mo.ui.dropdown(
        ...
        value=f"{gu.TENSE_LABELS['imperfect']['greek']} (Παρατατικός)",
        label="Επίλεξε χρόνο:",
    )
    tense_selector
...
def _(mo):
    # Adj file upload
    file_upload_adj = mo.ui.file(label="Φόρτωσε TSV επιθέτων")
    file_upload_adj
...
def _(df_adj, mo, tbl_sel_a):
    ...
    mo.vstack([mo.md("### Επίλεξε επίθετα για εξάσκηση"), table_adj if table_adj is not None else mo.md("_Το αρχείο adjectives.tsv δεν βρέθηκε — φόρτωσε αρχείο για να ξεκινήσεις._")])
    return (table_adj,)
