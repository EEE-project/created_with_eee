...
def _(mo):
    ...
    **Level:** B1 | **Theme:** Social Contacts & Communication
...
@app.cell(hide_code=True)
def _(mo):
    file_upload_noun = mo.ui.file(label="Upload nouns file (nouns.tsv) — optional", filetypes=[".tsv"])
    file_upload_noun
...
def _(df_noun, mo):
    ...
    mo.vstack([
        ...
        table_noun if table_noun is not None else mo.md("_nouns.tsv not found — upload a file to begin._")
    ])
    return (table_noun,)
...
def _(mo, noun_form, noun_trans, noun_word, words4test_noun, words_noun):
    ...
    if words4test_noun() and noun_word:
        ...
        _view = mo.md("_Select words above to start the test._")
    _view
    return
...
@app.cell(hide_code=True)
def _(mo):
    file_upload_verb = mo.ui.file(label="Upload verbs file (verbs.tsv) — optional", filetypes=[".tsv"])
    file_upload_verb
...
def _(df_verb, mo):
    ...
    mo.vstack([
        mo.md("### Select verbs to practice"),
        table_verb if table_verb is not None else mo.md("_verbs.tsv not found — upload a file to begin._")
    ])
    return (table_verb,)
...
def _(gu, mo, tense_selector, words4test_verb, words_verb):
    ...
    if cv_verb and _tense_key:
        ...
        _view = mo.vstack([
            mo.md(f"#### Verb test ({_total - _remaining + 1}/{_total})"),
            _verb_md,
        ])
    else:
        verb_form = None
...
def _(
    ...
    if cv_verb and tense_selector.value and verb_form is not None:
        ...
        if _ok:
            ...
            _total = len(words_verb)
            _msg = f'Test for <b>"{cv_verb["Word"]} — {cv_verb["Translation"]}"</b> passed.\n\n{_remaining} words remaining out of {_total}.'
            set_verb_msg(_msg)
...
@app.cell(hide_code=True)
def _(mo):
    file_upload_adj = mo.ui.file(label="Upload adjectives file (adjectives.tsv) — optional", filetypes=[".tsv"])
    file_upload_adj
...
def _(df_adj, mo):
    ...
    mo.vstack([
        mo.md("### Select adjectives to practice"),
        table_adj if table_adj is not None else mo.md("_adjectives.tsv not found — upload a file to begin._")
    ])
    return (table_adj,)
