...
def _(mo):
    ...
    Απλή Υποτακτική использует **ту же основу, что и Простое будущее** (Απλός Μέλλοντας), но с **να** или **ας** вместо **θα**.
...
def _(mo):
    ...
    **Αν** вводит условие. Глагол после **αν** использует основу Простого будущего **без θα**.
    ...
    | Условие (αν + основа Простого будущего) | Следствие |
    |:-------------------------------------|:-------|
    | **Αν** *έρθετε* στην Ελλάδα τον Ιούνιο, | *θα βρείτε* κάτι καλό και οικονομικό. |
    | **Αν** τη *δω*, | *θα* της το *πω*. |

    Придаточное следствия может использовать Простое будущее (**θα** + глагол) или Υποτακτική (**να** + глагол):
...
def _(mo):
    ...
    **Αν και** и **ενώ** означают «хотя / несмотря на то что»:
...
def _(mo):
    ...
    | Όλα εντάξει με το δωμάτιο; Είστε ικανοποιημένες; | Всё в порядке с номером? Вы довольны? |
    ...
    | Και ό,τι άλλο χρειαστείτε είμαι στη διάθεσή σας. | Если что-то понадобится, я в вашем распоряжении. |
