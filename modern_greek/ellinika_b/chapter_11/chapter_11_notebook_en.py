...
@app.cell(hide_code=True)
def _(mo):
    # Grammar: Simple Subjunctive
    mo.md("""
    ## Grammar: Simple Subjunctive (Απλή Υποτακτική)

    The Simple Subjunctive uses **the same stem as the Simple Future** (Απλός Μέλλοντας), but with **να** or **ας** instead of **θα**.
...
def _(mo):
    ...
    **Αν** introduces a condition. The verb after **αν** uses the Simple Future form **without θα**.

    | Condition (αν + Simple Future stem) | Result |
    |:-------------------------------------|:-------|
    | **Αν** *έρθετε* στην Ελλάδα τον Ιούνιο, | *θα βρείτε* κάτι καλό και οικονομικό. |
    | **Αν** τη *δω*, | *θα* της το *πω*. |

    The result clause can use either the Simple Future (**θα** + verb) or the Simple Subjunctive (**να** + verb):
...
def _(mo):
    ...
    | Όλα εντάξει με το δωμάτιο; Είστε ικανοποιημένες; | Is everything OK with the room? Are you satisfied? |
    ...
    | Και ό,τι άλλο χρειαστείτε είμαι στη διάθεσή σας. | And whatever else you need, I'm at your disposal. |
...
def _(df_noun, gu, mo, tbl_sel_n):
    ...
    mo.vstack([mo.md("### Select nouns to practice"), table_noun])
    return (table_noun,)
...
def _(gu, mo):
    ...
    tense_selector = mo.ui.dropdown(
        ...
        value=f"{gu.TENSE_LABELS['future']['greek']} (Subjunctive / Simple Future)",
        label="Select tense:",
    )
    tense_selector
