...
def _(language_selector, mo):
    ...
    **κανένας/κανείς, καμία/καμιά, κανένα** и **τίποτα** в утвердительных/вопросительных предложениях означают «кто-то/что-то», а в отрицательных — «никто/ничто».
    ...
    _«Καμιά φορά πάμε με κανέναν φίλο σου πουθενά να φάμε τίποτα;» (Иногда мы едем с каким-нибудь другом куда-нибудь, чтобы съесть что-нибудь?) — пример из учебника, где κανέναν/πουθενά/τίποτα все стоят в «утвердительном» (вопросительном) значении._
    """)
    elif _lang == 'el':
        _md = mo.md("""
...
def _(language_selector, mo):
    ...
    **κάτι** («что-то»):
    ...
    Окончание меняется во множественном числе, но ударение остаётся на том же слоге: κάποιος → κάποι**οι**, как и τέτοιος → τέτοι**οι**.
    """)
    elif _lang == 'el':
        _md = mo.md("""
    ...
    **κάτι**:
    ...
    Η κατάληξη αλλάζει στον πληθυντικό, αλλά ο τόνος παραμένει στην ίδια συλλαβή: κάποιος → κάποι**οι**, όπως και τέτοιος → τέτοι**οι**.
    """)
    else:
        _md = mo.md("""
    ...
    **κάτι** ("something"):
    ...
    The final ending changes in the plural, but the stress remains on the same syllable: κάποιος → κάποι**οι**, same as τέτοιος → τέτοι**οι**.
    """)
    _md
    return
...
def _(RAW_BASE, gu2, language_selector, notebook_dir):
    # Load pronoun data
    df_pron = gu2.load_vocab_table("pronouns.tsv", nb_dir=notebook_dir, remote_base=RAW_BASE, ru_variant=True, language=language_selector.value)
    return (df_pron,)


@app.cell(hide_code=True)
def _(df_pron, gu2, language_selector, mo, t_ui):
    # Pronoun table
    table_pron = gu2.vocab_table(df_pron)
    _lang = language_selector.value
...
def _(gu2, random, table_pron):
    ...
    errors_pron, set_errors_pron, retry_count_p, set_retry_count_p = gu2.make_error_tracking_state()
    return (
        captured_pron,
        enter_count_p,
        entered_pron,
...
@app.cell(hide_code=True)
def _(errors_pron, gu2, language_selector):
    # Pronoun retry-mistakes button
    retry_btn_p = gu2.retry_mistakes_button(errors_pron(), lang=language_selector.value)
    return (retry_btn_p,)
...
def _(
    ...
    gu2.pronoun_paradigm_drill_form(
        ...
        get_errors=errors_pron, set_errors=set_errors_pron,
        get_retry_cnt=retry_count_p, set_retry_cnt=set_retry_count_p,
        retry_btn=retry_btn_p,
...
def _():
    ...
    RAW_BASE = "https://raw.githubusercontent.com/EEE-project/created_with_eee/main/modern_greek/ellinika_b/chapter_03"
